...
export const createReport = (arr: IROrder[]): IReport[] => {
  return Object.values(
    arr.reduce((acc: { [key: string]: IReport }, order) => {
      ...
      const cash =
        order.payment_method === IPaymentMethod.CASH ? Number(order.total) : 0;
      const card =
      ...
      acc[key].beg += order.line_items.filter(
        (el) => el.name === NameOfClass.BEGINNER
      ).length;
      acc[key].adv += order.line_items.filter(
        (el) => el.name === NameOfClass.ADV
      ).length;
